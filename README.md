...
本サイトでは比較可能な企業情報をCSV、企業固有の編集本文をMDXで管理します。
これにより、「業界を知る」「企業を比較する」「企業を詳しく調べる」という流れで閲覧できるようにします。
...
LINEヤフー、リクルート、メルカリ、サイバーエージェントは、既存の `yukijya_doh` の記事を基にデータを移行します。DeNA以降の企業は新たに企業データを作成します。
...
Sakana AI と Preferred Networks は既存の `yukijya_doh` の記事を基にデータを移行します。ベンチャーは上場・未上場が混在し、未上場企業は有価証券報告書の提出義務がないため財務指標の法定開示がありません。そのため未上場企業は採用情報・事業内容などのプロフィールを中心に掲載し、財務の横断比較は上場企業のみ対象になります。
