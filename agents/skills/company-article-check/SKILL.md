...
description: 企業MDX記事を読み取り専用で監査し、構成・出典・新卒採用・事業・IRデータとの不整合に加え、用語・論証・文体・LLM由来の冗長さを行番号付きで報告する。「企業記事を監査して」「記事の品質を確認して」「記事の問題を洗い出して」「company-article-check」で使う。記事の執筆・更新依頼には使わない。
...
このスキルの [監査チェックリスト](references/checklist.md) と [企業記事の散文規範](references/prose-style.md)、および「正本」に挙げた資料を読む。判定に必要な規則は、このスキルのディレクトリと「正本」に挙げたリポジトリ内資料だけから取得する。各記事について、存在する範囲で次を読む。
...
`references/checklist.md` の A〜J 群を順に適用する。F7〜F9では用語の初出・定義・表記を確認し、「用語の定義確認」表へ記録する。H群・I群は読者に表示される散文へ、J群は表や注記を含む読者向けテキスト全体へ適用する。frontmatter、import、props、URL、コード上の識別子そのものを散文として評価しない。

条件付き要素は、企業の上場区分、開示状況、採用主体と開示主体、セグメント開示の有無を先に確認してから判定する。タグの有無だけでなく、渡された `companyId` とCSV行を追い、空欄、`確認中`、空の表・グラフが生成されないかまで確認する。

各項目を `OK` / `WARN` / `NG` / `該当無し` / `未評価` のいずれかで埋める。規約違反と、規約どおりか判断できない情報不足を混同しない。

**完了条件**: 対象記事とチェック項目の全組み合わせに判定があり、`WARN` / `NG` / `未評価` には記事ごとの根拠がある。「用語の定義確認」表に、少なくともF7〜F9の `WARN` / `NG` がすべて記録されている。
...
単一記事では全チェック項目を表示する。複数記事では、記事別集計と項目別集計を表示した後、`WARN` / `NG` / `未評価` をすべて列挙する。指摘は `NG`、`WARN`、`未評価` の順に並べる。単一・複数を問わず「用語の定義確認」表を出す。

**完了条件**: 各指摘にチェックID、引用、ファイルと行番号、違反した規約、観測した根拠、具体的な修正方針または追加で必要な根拠がある。同じ箇所を複数IDで重複して指摘せず、関連IDを1件の指摘へ併記している。
...
| 用語 | 初出 | 種別 | 判定 | コメント・推奨表記 |
| --- | --- | --- | --- | --- |
| `〈用語〉` | `site/src/pages/companies/example.mdx:Lxx` | 企業・業界固有語 | NG | 指す事業・製品・役割が未定義。初出で記事の理解に必要な範囲を説明する |
| 〈原語〉 | `site/src/pages/companies/example.mdx:Lyy` | 外来語 | WARN | 正確な原語を残す必要がないため、文脈に対応する平易な日本語へ置き換える |
...
- 企業記事の正本と散文規範が食い違う場合は「正本」の優先順位に従う。固定見出し、公式名称・直接引用、製品名、規格名、コード識別子、ディレクティブ内の敬体を、一般的な文体規範だけを理由に変更させない。
- 用語や文体の一般性を判断するために外部検索を行わない。ローカル根拠だけで適切な言い換えを確定できなければ、意味を推測せず `〈要再調査: この語が指す対象または操作〉` とする。
- 「用語の定義確認」表は必ず出力する。問題のある用語がない場合も省略せず、「該当用語なし」と記載する。
- H〜J群の指摘にも引用・位置・具体的な書き換え案を付ける。「LLMっぽい」「冗長」「読みにくい」だけの指摘を出さない。
- 記事の規約は推測・評価を禁じるため、H4の不確実性保持を推測の追加理由にしない。公式資料が示す条件・可能性・非開示の範囲は断定へ変えず、根拠のない推量はF3として除く。
- H4をJ2・J6より優先する。根拠に即した限定や不確実性は残し、中身のない弱い述語だけを圧縮する。
- 書き換え案そのものがJ群に違反しないようにする。修正案へ「重要なのは」「正面から」「多角的に」などの空虚な装飾を加えない。
...
- [企業記事の散文規範](references/prose-style.md): H〜J群の判定基準と企業記事向けの適用除外。
