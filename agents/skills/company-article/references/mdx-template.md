...
- `:::info` / `:::warn` / `:::error` の中は敬体でよい。読者への注意喚起なので、地の文と語り口が変わってよい箇所として扱う。
- 調査手順は台帳だけに残す。**誰が・いつ・どこで確かめたかを本文に書かない。**
- 出典はリンクにして事実へ直接張る。リンクを張る以上、情報源は自明なので前置きも後置きも要らない。
- **記事の完結性を保つ。リンクは根拠の所在であり、説明の代わりではない。** 事業、サービス、職種、待遇は、リンク先を開かずに分かる範囲を本文に書く。会社名・製品名にURLを張るだけで、それが何をするものかを書かない状態にしない。
