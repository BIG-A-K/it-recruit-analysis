...
- **既存記事を書き換えるときは情報を落とさない。** 削る文が持つ情報が記事の別の箇所に残るかを1文ずつ確認し、残らないなら削らない。文体・表記の統一は、文の主題を保ったまま行う。
- 段落や注記が担う意味づけ（読者にとっての位置づけ、なぜその事実を挙げるか）を、事実の言い換えへ置き換えない。`:::info` を本文や関連企業表にある事実の要約で埋めない。
...
**完了条件**: テンプレートの全セクションが配置され、セグメント説明、`SegmentRevenue`、関連企業がこの順に並び、各セクションに公式資料由来の内容がある。本文が常体で、調査実況・伝聞・確認日を含まない。省いたセクションは理由を記録している。既存記事を更新した場合は、書き換えで落とした情報を列挙し、それぞれが記事内の別の箇所に残ることを示せる。
...
- 既存ページを更新するとき、ユーザーが移行を求めない限りページ全体を機械的に書き換えない。文体・表記の統一だけを理由に、既存の記述が持つ意味づけを削らない。
