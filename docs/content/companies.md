...
| `SegmentRevenue` | 最新年度のセグメント別売上・利益。事業説明は直前のMDX本文へ書く |
...
セグメントの説明は`CompanyProse`へ一度だけ書き、その直後に`SegmentRevenue`を置きます。

## データがない企業

コンポーネントは、対象企業に表示できるデータがある場合だけMDXへ記述します。
非上場企業で法定開示の財務・雇用データがない場合、空欄表示のために
`EmploymentOverview`、`SegmentRevenue`、`HealthMetrics`、`FinancialHistory`、
